/* $Id: tabid.c,v 1.3 2019/05/28 15:24:56 prs Exp $ */
#include "tabid.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static struct id { int type; void *attrib; char *name; struct id *next; } *root = 0;
...
static int level;
static void IDadd(int typ, char *s, void *attrib) {
...
*/
int IDnew(int typ, char *s, void *attrib) {
  struct id *aux;
...
int IDinsert(int lev, int typ, char *s, void *attrib) {
  ...
  aux->attrib = attrib;
  *base = aux;
  if (IDdebug != 0) printf("#>>INSERT(%d)=%s (%d:%p)\n", lev, s, typ, attrib);
  return 1;
}

int IDreplace(int typ, char *s, void *attrib) {
  struct id *aux;
...
int IDsearch(char *s, void **attrib, int skip, int lev) {
  struct id *aux = root;
...
int IDforall(IDfunc f, void *user, int skip, int lev) {
  struct id *aux;
  int cnt = 0, ret;
...
void IDprint(int skip, int lev) {
  ...
  for (aux = root; aux != 0; aux = aux->next)
	...
	break; /* stop after 'lev' levels */
      ...
      printf(" %s:%d#%p", aux->name, aux->type, aux->attrib);
  printf("\n");
}
